...
bool UInventoryComponent::add(FItemBaseData item, int  numberOfQuestItems)
{
    ...
    int QuestQuantity = numberOfQuestItems;
    if (QuestQuantity > item.quantity)
    {
        QuestQuantity = item.quantity;
    }

    if (numberOfQuestItems > 0)
    {
        FItemBaseData QuestItem = FItemBaseData(&item);
        QuestItem.quantity = QuestQuantity;
        add(QuestItem, true);

    }
    if (item.quantity - QuestQuantity > 0)
    {

        FItemBaseData NormalItem = FItemBaseData(&item);
        NormalItem.quantity = item.quantity- QuestQuantity;
        return add(NormalItem,false);
    }
    return true;
}
